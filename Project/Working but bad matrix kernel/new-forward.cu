      ...
      if (input_h + p < H)
      {
        for (int q = 0; q < K; q++)
        {
          if (input_w + q < W)
          {
            ...
            //numBRows = unrolled_H
            input_unroll[b * (numBRows * numBColumns) + h_unroll * numBColumns + w_unroll] = in_4d(b, c, input_h + p, input_w + q);
            // B[b*numBColumns* numBRows + (q * TILE_WIDTH* numBColumns + ty* numBColumns + Col];
          }
        }
      }
    }
