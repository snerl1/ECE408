...
#define UNROLL_TILE_HEIGHT 32


// kernel to unroll input matrix to prepare it for matrix multiplication
__global__ void input_matrix_unroll(const float *input, __half *input_unroll, const int B, const int C, const int H, const int W, const int K, const int S)
{
#define in_4d(i3, i2, i1, i0) input[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]
  ...
  int transposed_h = h_unroll - c * K2;
  ...
  if (h_unroll  < unrolled_H && w_unroll < unrolled_W && input_h + p < H && input_w + q < W)
  {
    __half temp = __float2half(in_4d(blockIdx.z, c, input_h + p, input_w + q));
    input_unroll[blockIdx.z * (unrolled_H * unrolled_W) + h_unroll * unrolled_W + w_unroll] = temp;
  }
}

// kernel to perform shared memory matrix multiplication
__global__ void matrixMultiplyShared(const float *A, const __half *B, float *C,
                                     ...
                                     int numCRows, int numCColumns)
{
  ...
  for (int q = 0; q < ((numAColumns + TILE_WIDTH - 1) / TILE_WIDTH); ++q)
  {
    ...
    if (Row < numARows && (q * TILE_WIDTH + tx) < numAColumns)
      ...
      subTileA[ty][tx] = __float2half(0.0);
    if (Col < numBColumns && (q * TILE_WIDTH + ty) < numBRows)
      subTileB[ty][tx] = B[b*numBColumns* numBRows + (q * TILE_WIDTH + ty) * numBColumns + Col];
    else
      subTileB[ty][tx] = __float2half(0.0);
    __syncthreads();
    for (int k = 0; k < TILE_WIDTH; ++k)
      Pvalue = __hadd(Pvalue, __hmul(subTileA[ty][k], subTileB[k][tx]));
    __syncthreads();
  }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
  ...
  int W_size = (unrolled_W + UNROLL_TILE_WIDTH - 1) / UNROLL_TILE_WIDTH;
  int H_size = (unrolled_H + UNROLL_TILE_HEIGHT - 1) / UNROLL_TILE_HEIGHT;

  __half *device_input_unroll;
  cudaMalloc(&device_input_unroll, B * unrolled_H * unrolled_W * sizeof(__half));

  dim3 blockDim(UNROLL_TILE_WIDTH, UNROLL_TILE_HEIGHT, 1);
